...
```bash
git clone https://github.com/RmysaDmr/GrupNo_6.git
cd GanttNo_6
```
Adım 3: Sanal Ortam Oluşturma (Opsiyonel): Proje bağımlılıklarının bilgisayarınızdaki diğer projelerle çakışmaması için PyCharm içinde veya terminalde bir sanal ortam (venv) oluşturabilirsiniz:
```bash
python -m venv venv
```
Adım 4: Gerekli Paketlerin Yüklenmesi: Programın matematiksel hesaplama ve grafik motorunun çalışması için gereken paketleri pip paket yöneticisiyle sisteme kurun:
```bash
pip install numpy matplotlib
```
Adım 5: Programın Tetiklenmesi: Kurulum tamamlandıktan sonra ana scripti çalıştırarak dinamik veri giriş panelini başlatın:
```bash
